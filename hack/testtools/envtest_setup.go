...
func (te *TestEnvironment) VerifyCRDInstallation() error {
	expectedCRDs := []string{
		"networkintents.intent.nephoran.com",
	}

	ctx, cancel := context.WithTimeout(te.ctx, 60*time.Second)
	defer cancel()
...
func (te *TestEnvironment) verifySingleCRD(ctx context.Context, crdName string) error {
	Eventually(func() bool {
		if strings.Contains(crdName, "networkintents") {
			niList := &intentv1alpha1.NetworkIntentList{}
			err := te.K8sClient.List(ctx, niList)
			return err == nil || !errors.IsNotFound(err)
		}
		return true // Skip verification for unimplemented CRDs
...
func (te *TestEnvironment) WaitForResourceReady(obj client.Object, timeout time.Duration) error {
	...
	Eventually(func() bool {
		...
		return err == nil
	}, timeout, 100*time.Millisecond).Should(BeTrue(), "resource should be ready")
...
func (te *TestEnvironment) cleanupTestResources() {
	...
	if err := te.K8sClient.List(ctx, namespaceList, client.MatchingLabels(labels)); err == nil {
		for _, ns := range namespaceList.Items {
			te.cleanupNamespaceResources(ctx, ns.Name)
		}
	}

	// Also cleanup test resources in default namespace
	te.cleanupNamespaceResources(ctx, te.options.DefaultNamespace)
}

// cleanupNamespaceResources cleans up resources in a specific namespace.
func (te *TestEnvironment) cleanupNamespaceResources(ctx context.Context, namespace string) {
	// Cleanup NetworkIntents
	networkIntentList := &intentv1alpha1.NetworkIntentList{}
	...
	te.cleanupPods(ctx, namespace)
	te.cleanupDeployments(ctx, namespace)
	te.cleanupServices(ctx, namespace)
	te.cleanupConfigMaps(ctx, namespace)
	te.cleanupSecrets(ctx, namespace)
}

// Individual resource cleanup functions
func (te *TestEnvironment) cleanupPods(ctx context.Context, namespace string) {
...
func (te *TestEnvironment) cleanupDeployments(ctx context.Context, namespace string) {
...
func (te *TestEnvironment) cleanupServices(ctx context.Context, namespace string) {
...
func (te *TestEnvironment) cleanupConfigMaps(ctx context.Context, namespace string) {
...
func (te *TestEnvironment) cleanupSecrets(ctx context.Context, namespace string) {
