...
func analyzeFlakiness(allResults map[string][]TestResult) FlakeReport {
	report := FlakeReport{
		...
		Summary: Summary{
			HighFlakeTests:      make([]string, 0),
			MediumFlakeTests:    make([]string, 0),
			LowFlakeTests:       make([]string, 0),
			ConsistentlyFailing: make([]string, 0),
			ConsistentlyPassing: make([]string, 0),
		},
	}
