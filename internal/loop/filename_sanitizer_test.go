...
func TestSanitizeStatusFilename_EdgeCasesComprehensive(t *testing.T) {
	edgeCases := []struct {
		...
		{"OnlyUnicode", "ÊµãË??á‰ª∂", true, "only Unicode should get default name"},
		{"VeryLongFilename", strings.Repeat("a", 500), true, "very long should be truncated"},
		{"LongUnicode", strings.Repeat("Êµ?, 200), true, "long Unicode should be handled"},
		...
		{"MixedComplexity", "C:\\ÊµãË?\\<file|name>with??ÆÈ?*.tmp~", true, "complex mixed input should be handled"},
...
func BenchmarkSanitizeStatusFilename_ComprehensivePerformance(b *testing.B) {
	benchmarkCases := []struct {
		...
		{"UnicodeHeavy", "intent-ÊµãË?-?–∞–π–ª-?ï„Ç°?§„É´.json"},
		{"WindowsReservedDevice", "CON.json"},
		{"VeryLongFilename", strings.Repeat("intent-test-", 50) + ".json"},
		{"ComplexMixed", "C:\\ÊµãË?\\<file|name>with??ÆÈ?*.tmp~.json"},
		{"OnlyUnicode", "ÊµãË??á‰ª∂?çÁß∞"},
