...
// LoadLLMAPIKeyFromFile loads LLM API key from file or environment variable
// This function checks for backend-specific API key files and fallbacks
func LoadLLMAPIKeyFromFile(backendType string, auditLogger interface{}) (string, error) {
	// First try backend-specific environment variable
	var envKeys []string
	switch backendType {
	case "openai":
		envKeys = []string{"OPENAI_API_KEY", "LLM_API_KEY"}
	case "anthropic":
		envKeys = []string{"ANTHROPIC_API_KEY", "LLM_API_KEY"} 
	case "azure":
		envKeys = []string{"AZURE_OPENAI_API_KEY", "LLM_API_KEY"}
	default:
	...
	// Try file paths
	filePaths := []string{
		"/etc/secrets/llm-api-key",
		"/var/secrets/llm-api-key", 
		"/secrets/llm-api-key",
		fmt.Sprintf("/etc/secrets/%s-api-key", backendType),
	}
	...
	return "", fmt.Errorf("LLM API key not found in environment or file system for backend: %s", backendType)
}

// LoadAPIKeyFromFile loads API key from file or environment variable
func LoadAPIKeyFromFile(auditLogger interface{}) (string, error) {
	...
	return "", fmt.Errorf("API key not found in environment or file system")
}

// LoadJWTSecretKeyFromFile loads JWT secret key from file or environment variable  
func LoadJWTSecretKeyFromFile(auditLogger interface{}) (string, error) {
	// Try environment variable first
	...
	// Try file paths
	filePaths := []string{
		"/etc/secrets/jwt-secret-key",
		"/var/secrets/jwt-secret-key", 
	...
	return "", fmt.Errorf("JWT secret key not found in environment or file system")
}