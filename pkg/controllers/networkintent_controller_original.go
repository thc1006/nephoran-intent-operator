...
func NewNetworkIntentReconciler(client client.Client, scheme *runtime.Scheme, deps Dependencies, config *Config) (*NetworkIntentReconciler, error) {
	...
	// Initialize LLM sanitizer with secure defaults
	...
	timeoutConfig := &resilience.TimeoutConfig{
		...
		GitTimeout:               60 * time.Second,  // Git operations timeout
		KubernetesTimeout:        30 * time.Second,  // K8s API timeout
		PackageGenerationTimeout: 120 * time.Second, // Package generation timeout
		RAGTimeout:               15 * time.Second,  // RAG retrieval timeout
		ReconciliationTimeout:    300 * time.Second, // Overall reconciliation timeout
		DefaultTimeout:           30 * time.Second,  // Default for other ops
	}
...
func (r *NetworkIntentReconciler) processLLMPhase(ctx context.Context, networkIntent *nephoranv1.NetworkIntent, processingCtx *ProcessingContext) (ctrl.Result, error) {
	...
	sanitizedIntent, err := r.llmSanitizer.SanitizeInput(ctx, networkIntent.Spec.Intent)
	if err != nil {
		...
		updateCondition(&networkIntent.Status.Conditions, condition)
		r.setReadyCondition(ctx, networkIntent, metav1.ConditionFalse, "SecurityValidationFailed", fmt.Sprintf("Intent failed security validation: %v", err))
		r.recordFailureEvent(networkIntent, "SecurityValidationFailed", err.Error())
	...
	enhancedPrompt, err := r.buildTelecomEnhancedPrompt(ctx, sanitizedIntent, processingCtx)
	if err != nil {
	...
	securePrompt := r.llmSanitizer.BuildSecurePrompt(systemPrompt, enhancedPrompt)
	...
	result, err := r.llmCircuitBreaker.ExecuteWithFallback(ctx,
		...
		func(ctx context.Context) (interface{}, error) {
			return r.timeoutManager.ExecuteWithTimeout(ctx, resilience.OperationTypeLLM,
	...
	validatedOutput, err := r.llmSanitizer.ValidateOutput(ctx, processedResult)
	if err != nil {
		...
		updateCondition(&networkIntent.Status.Conditions, condition)
		r.setReadyCondition(ctx, networkIntent, metav1.ConditionFalse, "OutputSecurityValidationFailed", fmt.Sprintf("LLM output failed security validation: %v", err))
		r.recordFailureEvent(networkIntent, "OutputSecurityValidationFailed", err.Error())
...
func (r *NetworkIntentReconciler) commitToGitOps(ctx context.Context, networkIntent *nephoranv1.NetworkIntent, processingCtx *ProcessingContext) (ctrl.Result, error) {
	...
	_, err := r.timeoutManager.ExecuteWithTimeout(ctx, resilience.OperationTypeGit,
		func(timeoutCtx context.Context) (interface{}, error) {
	...
	result, err := r.timeoutManager.ExecuteWithTimeout(ctx, resilience.OperationTypeGit,
		func(timeoutCtx context.Context) (interface{}, error) {
...
func (r *NetworkIntentReconciler) generateFallbackResponse(intent string) string {
	...
	if strings.Contains(intentLower, "amf") || strings.Contains(intentLower, "access and mobility") {
		...
		response = map[string]interface{}{
			...
			"interfaces": []string{"F1-C", "F1-U", "Open-Fronthaul"},
		}
		...
		response = map[string]interface{}{
			...
			"interfaces": []string{"F1-C", "F1-U", "X2", "Xn"},
		}
	} else {
		// Generic fallback for unrecognized intents
