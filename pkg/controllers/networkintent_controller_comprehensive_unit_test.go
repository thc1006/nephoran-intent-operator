...
func createTestNetworkIntent(name, namespace, intent string) *intentv1alpha1.NetworkIntent {
	return &intentv1alpha1.NetworkIntent{
		...
		Status: intentv1alpha1.NetworkIntentStatus{
			Phase: "Pending",
		},
	}
}
...
func TestReconcile(t *testing.T) {
	scheme := runtime.NewScheme()
	intentv1alpha1.AddToScheme(scheme)
	...
	tests := []struct {
		...
		validationCheck func(t *testing.T, ni *intentv1alpha1.NetworkIntent)
	}{
		{
			...
			validationCheck: func(t *testing.T, ni *intentv1alpha1.NetworkIntent) {
				...
				assert.True(t, ni.Status.Phase == "Deployed" || 
						   ni.Status.Phase == "Processing" ||
						   ni.Status.Phase == "Active")
			},
		},
		{
			...
			validationCheck: func(t *testing.T, ni *intentv1alpha1.NetworkIntent) {
				...
				assert.True(t, ni.Status.Phase == "Processing" ||
						   ni.Status.Phase == "Active" ||
		...
		},
		{
			...
			validationCheck: func(t *testing.T, ni *intentv1alpha1.NetworkIntent) {
				assert.Equal(t, "Failed", ni.Status.Phase)
			},
		},
		{
		...
		},
		{
			...
			networkIntent: func() *intentv1alpha1.NetworkIntent {
			...
			expectedResult:  ctrl.Result{},
			expectedError:   false,
			expectedPhase:   "Pending", // Phase before deletion
			validationCheck: func(t *testing.T, ni *intentv1alpha1.NetworkIntent) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.validationCheck != nil && !tt.expectedError {
				updatedNI := &intentv1alpha1.NetworkIntent{}
