...
type ResourcePlannerInterface interface {
	...
	PlanNetworkFunction(nfName string, llmParams map[string]interface{}, telecomContext map[string]interface{}) (*PlannedNetworkFunction, error)
...
type StatusManagerInterface interface {
	...
	SetReadyCondition(ctx context.Context, networkIntent *nephoranv1.NetworkIntent, status string, reason, message string) error
	
	// UpdateCondition updates a specific condition
	UpdateCondition(networkIntent *nephoranv1.NetworkIntent, conditionType string, status string, reason, message string)
