	...
	/*
		...
		for i := 0; i < 20; i++ {
			...
			go func(i int) {
				...
				os.WriteFile(tempFile, []byte("data"), 0640)
...
func ExampleStandardCleanup(t *testing.T) {
	...
	/*
		...
		err := os.WriteFile(tempFile, []byte("data"), 0640)
	...
	/*
		...
		for i := 0; i < 5; i++ {
			...
			t.Run(fmt.Sprintf("test-%d", i), func(t *testing.T) {
				...
				os.WriteFile(configFile, []byte(`{"key": "value"}`), 0640)



				dataFile := filepath.Join(tempDir, "data.txt")

				os.WriteFile(dataFile, []byte("test data"), 0640)
